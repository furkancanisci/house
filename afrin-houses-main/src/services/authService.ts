...
export const authService: AuthService = {
  ...
  async updateUser(userData: Partial<User>): Promise<{ user: User; message: string }> {
    try {
      ...
      console.log('API Base URL:', process.env?.VITE_API_BASE_URL || 'https://house-6g6m.onrender.com/api/v1');
      
      // Let's log the full request details
      const requestUrl = `${process.env?.VITE_API_BASE_URL || 'https://house-6g6m.onrender.com/api/v1'}/dashboard/profile`;
