...
import { LayoutGrid, List, Loader2, DollarSign, Filter } from 'lucide-react';

/**
 * Houses for Sale Page Component
 * 
 * This component displays all properties available for sale with:
...
const HousesForSale: React.FC = () => {
  ...
  const [localFilters, setLocalFilters] = useState<SearchFilters>({
    listingType: 'sale', // Always filter for sale properties
    sortBy: 'created_at',
    sortOrder: 'desc'
  });

  // Filter properties to show only sale properties
  ...
  const handleFilterChange = (filters: SearchFilters) => {
    setLocalFilters(prev => ({
      ...prev,
      ...filters,
      listingType: 'sale' // Always maintain sale filter
    }));
  };
        ...
        {showFilters && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8 transform transition-all duration-300 ease-in-out">
            <SearchFiltersComponent
              ...
              hideListingType={true} // Hide listing type since we're already filtering for sale
            />
          </div>
        )}
