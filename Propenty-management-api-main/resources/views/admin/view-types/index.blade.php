...
@section('title', __('View Types Management'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('View Types Management') }} / {{ __('نوع الإطلالة') }}</h3>
                    <div class="card-tools">
                        <a href="{{ route('admin.view-types.create') }}" class="btn btn-primary btn-sm">
                            <i class="fas fa-plus"></i> {{ __('Add New View Type') }}
                        </a>
                    </div>
                </div>
                ...
                <div class="card-body">
                        ...
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="custom-control custom-checkbox">
                                            <input class="custom-control-input row-checkbox" type="checkbox"
                                                   id="checkbox_{{ $viewType->id }}" value="{{ $viewType->id }}">
                                            <label for="checkbox_{{ $viewType->id }}" class="custom-control-label"></label>
                                        </div>
                                    </td>
                                    <td>{{ $viewType->id }}</td>
                                    <td>
                                        <code class="text-primary">{{ $viewType->value }}</code>
                                    </td>
                                    <td>
                                        <strong>{{ $viewType->name_en }}</strong>
                                        <br>
                                        <small class="text-muted">{{ __('Created') }}: {{ $viewType->created_at->format('M d, Y') }}</small>
                                    </td>
                                    <td dir="rtl">
                                        <strong>{{ $viewType->name_ar }}</strong>
                                        <br>
                                        <small class="text-muted">{{ __('Updated') }}: {{ $viewType->updated_at->format('M d, Y') }}</small>
                                    </td>
                                    <td>
                                        <div class="description-preview">
                                            {{ Str::limit($viewType->description_en ?: $viewType->description_ar, 50) }}
                                            @if(strlen($viewType->description_en ?: $viewType->description_ar) > 50)
                                                <button type="button" class="btn btn-link btn-sm p-0"
                                                        onclick="showFullDescription('{{ $viewType->id }}')">
                                                    {{ __('Read more') }}
                                                </button>
                                            @endif
                                        </div>
                                    ...
                                    <td>
                                        <span class="badge badge-secondary badge-lg">{{ $viewType->sort_order }}</span>
                                    </td>
                                    <td>
                                        <span class="badge badge-info">{{ $viewType->properties_count ?? 0 }}</span>
                                        @if(($viewType->properties_count ?? 0) > 0)
                                            <br>
                                            <a href="{{ route('admin.properties.index', ['view_type' => $viewType->id]) }}"
                                    ...
                                    <td>
                                        <form action="{{ route('admin.view-types.toggle-status', $viewType) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-{{ $viewType->is_active ? 'success' : 'secondary' }}"
                                                    title="{{ $viewType->is_active ? __('Active - Click to deactivate') : __('Inactive - Click to activate') }}">
                                                <i class="fas fa-{{ $viewType->is_active ? 'check' : 'times' }}"></i>
                                                {{ $viewType->is_active ? __('Active') : __('Inactive') }}
                                            </button>
                                        </form>
                                    </td>
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('admin.view-types.show', $viewType) }}"
                                               class="btn btn-info btn-sm" title="{{ __('View') }}">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.view-types.edit', $viewType) }}"
                                               class="btn btn-primary btn-sm" title="{{ __('Edit') }}">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @if(($viewType->properties_count ?? 0) == 0)
                                            <button type="button" class="btn btn-danger btn-sm"
                                                    onclick="confirmDelete({{ $viewType->id }})" title="{{ __('Delete') }}">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                            @else
                                            <button type="button" class="btn btn-secondary btn-sm" disabled
                                                    title="{{ __('Cannot delete: View type is used by properties') }}">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                            @endif
                                        </div>
                                ...
                                <tr>
                                    <td colspan="10" class="text-center py-4">
                                        <div class="empty-state">
                                            ...
                                            <h5 class="text-muted">{{ __('No view types found') }}</h5>
                                            <p class="text-muted">{{ __('Start by creating your first view type') }}</p>
                                            <a href="{{ route('admin.view-types.create') }}" class="btn btn-primary">
                                                <i class="fas fa-plus"></i> {{ __('Add New View Type') }}
                                            </a>
                                        </div>
                                    </td>
                    ...
                    <!-- Pagination -->
                    @if($viewTypes->hasPages())
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div>
                            <p class="text-muted">
                                {{ __('Showing :from to :to of :total results', [
                                    'from' => $viewTypes->firstItem(),
                                    'to' => $viewTypes->lastItem(),
                                    'total' => $viewTypes->total()
                                ]) }}
                            </p>
                        </div>
                        <div>
                            {{ $viewTypes->appends(request()->query())->links() }}
                        </div>
                    </div>
                    @endif
                </div>
...
<form id="delete-form-{{ $viewType->id }}" action="{{ route('admin.view-types.destroy', $viewType) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
</form>
@endforeach
...
function applyFilters() {
    ...
    window.location.href = '{{ route('admin.view-types.index') }}?' + params.toString();
}
...
function performBulkAction(action, actionText) {
    ...
    if (ids.length === 0) {
        toastr.warning('{{ __('Please select at least one view type') }}');
        return;
    }

    const confirmText = action === 'delete' ?
        '{{ __('This action cannot be undone!') }}' :
        '{{ __('Are you sure you want to :action the selected view types?', ['action' => ':action']) }}'.replace(':action', actionText);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.action = '{{ route('admin.view-types.bulk-action') }}';
            form.submit();
        }
    });
}
...
function showFullDescription(id) {
    ...
    const descriptions = @json($viewTypes->pluck('description_en', 'id')->merge($viewTypes->pluck('description_ar', 'id')));
...
$('form[action*="toggle-status"]').on('submit', function(e) {
    ...
    Swal.fire({
        ...
        text: '{{ __('Are you sure you want to :action this view type?', ['action' => ':action']) }}'.replace(':action', action),
        icon: 'question',
        showCancelButton: true,
...
$(document).on('keydown', function(e) {
    // Ctrl+N for new view type
    if (e.ctrlKey && e.key === 'n') {
        e.preventDefault();
        window.location.href = '{{ route('admin.view-types.create') }}';
    }
