...
@section('title', __('Floor Types Management'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('Floor Types Management') }}</h3>
                    <div class="card-tools">
                        <a href="{{ route('admin.floor-types.create') }}" class="btn btn-primary btn-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                @forelse($floorTypes as $floorType)
                                <tr>
                                    <td>
                                        <div class="custom-control custom-checkbox">
                                            <input class="custom-control-input row-checkbox" type="checkbox" 
                                                   id="checkbox_{{ $floorType->id }}" value="{{ $floorType->id }}">
                                            <label for="checkbox_{{ $floorType->id }}" class="custom-control-label"></label>
                                        </div>
                                    </td>
                                    <td>{{ $floorType->id }}</td>
                                    <td>
                                        <strong>{{ $floorType->name_en }}</strong>
                                        <br>
                                        <small class="text-muted">{{ __('Created') }}: {{ $floorType->created_at->format('M d, Y') }}</small>
                                    </td>
                                    <td dir="rtl">
                                        <strong>{{ $floorType->name_ar }}</strong>
                                        <br>
                                        <small class="text-muted">{{ __('Updated') }}: {{ $floorType->updated_at->format('M d, Y') }}</small>
                                    </td>
                                    <td>
                                        <div class="description-preview">
                                            {{ Str::limit($floorType->description_en ?: $floorType->description_ar, 50) }}
                                            @if(strlen($floorType->description_en ?: $floorType->description_ar) > 50)
                                                <button type="button" class="btn btn-link btn-sm p-0" 
                                                        onclick="showFullDescription('{{ $floorType->id }}')">
                                                    {{ __('Read more') }}
                                                </button>
                                            @endif
                                        </div>
                                    ...
                                    <td>
                                        <span class="badge badge-secondary badge-lg">{{ $floorType->sort_order }}</span>
                                    </td>
                                    <td>
                                        <span class="badge badge-info">{{ $floorType->properties_count ?? 0 }}</span>
                                        @if(($floorType->properties_count ?? 0) > 0)
                                            <br>
                                            <a href="{{ route('admin.properties.index', ['floor_type' => $floorType->id]) }}" 
                                    ...
                                    <td>
                                        <form action="{{ route('admin.floor-types.toggle-status', $floorType) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-{{ $floorType->is_active ? 'success' : 'secondary' }}" 
                                                    title="{{ $floorType->is_active ? __('Active - Click to deactivate') : __('Inactive - Click to activate') }}">
                                                <i class="fas fa-{{ $floorType->is_active ? 'check' : 'times' }}"></i>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('admin.floor-types.show', $floorType) }}" 
                                               class="btn btn-info btn-sm" title="{{ __('View') }}">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.floor-types.edit', $floorType) }}" 
                                               class="btn btn-primary btn-sm" title="{{ __('Edit') }}">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            @if(($floorType->properties_count ?? 0) == 0)
                                            <button type="button" class="btn btn-danger btn-sm" 
                                                    onclick="confirmDelete({{ $floorType->id }})" title="{{ __('Delete') }}">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                            @else
                                            <button type="button" class="btn btn-secondary btn-sm" disabled 
                                                    title="{{ __('Cannot delete: Floor type is used by properties') }}">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                            @endif
                                        </div>
                                ...
                                <tr>
                                    <td colspan="9" class="text-center py-4">
                                        <div class="empty-state">
                                            ...
                                            <h5 class="text-muted">{{ __('No floor types found') }}</h5>
                                            <p class="text-muted">{{ __('Start by creating your first floor type') }}</p>
                                            <a href="{{ route('admin.floor-types.create') }}" class="btn btn-primary">
                                                <i class="fas fa-plus"></i> {{ __('Add New Floor Type') }}
                                            </a>
                                        </div>
                                    </td>
                    ...
                    <!-- Pagination -->
                    @if($floorTypes->hasPages())
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <div>
                            <p class="text-muted">
                                {{ __('Showing :from to :to of :total results', [
                                    'from' => $floorTypes->firstItem(),
                                    'to' => $floorTypes->lastItem(),
                                    'total' => $floorTypes->total()
                                ]) }}
                            </p>
                        </div>
                        <div>
                            {{ $floorTypes->appends(request()->query())->links() }}
                        </div>
                    </div>
                    @endif
                </div>
...
<form id="delete-form-{{ $floorType->id }}" action="{{ route('admin.floor-types.destroy', $floorType) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
</form>
@endforeach
...
function applyFilters() {
    ...
    window.location.href = '{{ route('admin.floor-types.index') }}?' + params.toString();
}
...
function performBulkAction(action, actionText) {
    ...
    if (ids.length === 0) {
        toastr.warning('{{ __('Please select at least one floor type') }}');
        return;
    }
    
    const confirmText = action === 'delete' ? 
        '{{ __('This action cannot be undone!') }}' : 
        '{{ __('Are you sure you want to :action the selected floor types?', ['action' => ':action']) }}'.replace(':action', actionText);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.action = '{{ route('admin.floor-types.bulk-action') }}';
            form.submit();
        }
    });
}
...
function showFullDescription(id) {
    ...
    const descriptions = @json($floorTypes->pluck('description_en', 'id')->merge($floorTypes->pluck('description_ar', 'id')));
...
$('form[action*="toggle-status"]').on('submit', function(e) {
    ...
    Swal.fire({
        ...
        text: '{{ __('Are you sure you want to :action this floor type?', ['action' => ':action']) }}'.replace(':action', action),
        icon: 'question',
        showCancelButton: true,
