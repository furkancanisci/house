...
@section('title', __('Add New Floor Type'))
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('Add New Floor Type') }}</h3>
                ...
                <form action="{{ route('admin.floor-types.store') }}" method="POST" id="floor-type-form">
                    @csrf
                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <div class="preview-card">
                                                    <h6 id="preview_name_en" class="text-primary">{{ __('Floor Type Name') }}</h6>
                                                    <p id="preview_description_en" class="text-muted">{{ __('Floor type description will appear here') }}</p>
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="preview-card" dir="rtl">
                                                    <h6 id="preview_name_ar" class="text-success">{{ __('اسم نوع الأرضية') }}</h6>
                                                    <p id="preview_description_ar" class="text-muted">{{ __('سيظهر وصف نوع الأرضية هنا') }}</p>
                                                    <small class="text-info">
                                                        <i class="fas fa-sort-numeric-up"></i> 
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <button type="submit" class="btn btn-primary btn-lg">
                                    <i class="fas fa-save"></i> {{ __('Create Floor Type') }}
                                </button>
                            ...
                            <div class="col-md-6 text-right">
                                ...
                                <a href="{{ route('admin.floor-types.index') }}" class="btn btn-outline-secondary ml-2">
...
<form id="save-and-add-form" action="{{ route('admin.floor-types.store') }}" method="POST" style="display: none;">
    @csrf
...
$(document).ready(function() {
    ...
    $('#description_en').on('input', function() {
        const value = $(this).val() || '{{ __('Floor type description will appear here') }}';
        $('#preview_description_en').text(value);
        updateCharacterCount('description_en');
    });
    
    $('#description_ar').on('input', function() {
        const value = $(this).val() || '{{ __('سيظهر وصف نوع الأرضية هنا') }}';
...
function resetForm() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            document.getElementById('floor-type-form').reset();
            
            // Reset previews
            $('#preview_name_en').text('{{ __('Floor Type Name') }}');
            $('#preview_name_ar').text('{{ __('اسم نوع الأرضية') }}');
            $('#preview_description_en').text('{{ __('Floor type description will appear here') }}');
            $('#preview_description_ar').text('{{ __('سيظهر وصف نوع الأرضية هنا') }}');
            ...
            localStorage.removeItem('floor_type_form_data');
...
function saveFormData() {
    ...
    localStorage.setItem('floor_type_form_data', JSON.stringify(formData));
}

function loadFormData() {
    const savedData = localStorage.getItem('floor_type_form_data');
    if (savedData) {
